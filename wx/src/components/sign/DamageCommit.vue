      ...
      <div class="contact">
        <div v-for="(item,index) in damage.detail">
          ...
          <div class="damage-img-box">
            <div class="damage-null" @click="imgUp(index)" v-if="item.wxSrc == ''"></div>
            <div class="damage-img" v-if="item.wxSrc != ''" @click="previewImg(index)"><img :src="item.wxSrc"/></div>
            <div class="damage-close" v-if="item.wxSrc != ''" @click="imgDel(item)"></div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        ...
        // let _commitDetail = JSON.parse(sessionStorage.getItem('commitDetail'));
        let _commitDetail = self.$api.ssGet('commitDetail');
        self.$vux.confirm.show({
          content: '确认质损信息',
          confirmText: '是',
          cancelText: '否',
          ...
          onConfirm() {
            ...
            let arr = [];
            // let _damageArr = JSON.parse(sessionStorage.getItem('damageArr'));
            let _damageArr = self.$api.ssGet('damageArr');
            for (var i = 0; i < len; i++) {
              let obj = {
                id: null,
                reason: self.damage.detail[i].damageText,
                receiveId: _commitDetail.receiveId,
            ...
            self.$api.post("crm/sign/editVehicleLose", {operateCode: '148', operateName: '/质损中心/确认质损',vehicleRegister: arr}, r => {
              _damageArr[_commitDetail.index].isLose = 1;
              self.$api.ssSet('damageArr', _damageArr);
              sessionStorage.removeItem('commitDetail');
              self.$router.push({
