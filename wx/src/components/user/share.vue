...
export default {
  ...
  mounted() {
    ...
    self.$api.get(
      "/api/weixin/sign?url=" + encodeURIComponent(location.href.split("#")[0]),
      p,
      r => {
        wx.config({
          debug: false,
        ...
        wx.ready(function() {
          let shareData = {
            ...
            link:
              "http://srcwap.anji-plus.com/share/uv?fromOpenId=" + self.openId, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
